...
GATKs=/data1/apps/GATK/GenomeAnalysisTK-3.6
...
bcftools=/data1/apps/samtools/bcftools-1.3
Java7=/usr/bin/java
...
################################################################################
####  VEP - Annotation of Variants   #############################
VEP_v=83
ASSEMBLY=GRCh37
VEP_BIN_DIR=/data1/apps/VEP/ensembl-tools-release-"$VEP_v"/scripts/variant_effect_predictor
VEP_CACHE=$HOME/.vep/
...
cp $sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.indel.vcf $MyOutDir/$SampleID/Target/HaplotypeCaller/$onTarget_BAM.final.HaplotypeCaller.indel.vcf
		...
		inVEP=$sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.indel.vcf
		outVEP=$sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.indel.VEP.vcf
		out_tableize=$sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.indel.VEP.vcf.table.gz
...
perl  $VEP_BIN_DIR/variant_effect_predictor.pl \
		--offline --cache --no_progress --fork $nt \
		--dir_cache $VEP_CACHE --cache_version $VEP_v \
		--assembly $ASSEMBLY --fork $nt --fasta $FASTA \
		-i $inVEP  -o $outVEP \
		--allele_number --hgvs --everything \
...
cp $sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.snp.vcf $MyOutDir/$SampleID/Target/HaplotypeCaller/$onTarget_BAM.final.HaplotypeCaller.snp.vcf
		...
		inVEP=$sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.snp.vcf
		outVEP=$sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.snp.VEP.vcf
		out_tableize=$sftp/$RunName/HaplotypeCaller/$SampleID/$onTarget_BAM.final.HaplotypeCaller.snp.VEP.vcf.table.gz
...
cp $sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.indel.vcf $MyOutDir/$SampleID/Target/UnifiedGenotyper/$onTarget_BAM.final.UnifiedGenotyper.indel.vcf
		...
		inVEP=$sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.indel.vcf
		outVEP=$sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.indel.VEP.vcf
		out_tableize=$sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.indel.VEP.vcf.table.gz
...
perl  $VEP_BIN_DIR/variant_effect_predictor.pl \
		...
		--assembly $ASSEMBLY --fork $nt --fasta $FASTA \
		-i $inVEP  -o $outVEP \
		--allele_number --hgvs --everything --fields \ ALLELE_NUM,Consequence,SYMBOL,SYMBOL_SOURCE,Gene,Feature,BIOTYPE,HGVSc,HGVSp,cDNA_position,CDS_position,Protein_position,Amino_acids,Codons,Existing_variation,STRAND,CANONICAL,CCDS,ENSP,SIFT,PolyPhen,ExAC_MAF,PUBMED  \
		--vcf --force_overwrite
...
cp $sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.snp.vcf $MyOutDir/$SampleID/Target/UnifiedGenotyper/$onTarget_BAM.final.UnifiedGenotyper.snp.vcf
		...
		inVEP=$sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.snp.vcf
		outVEP=$sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.snp.VEP.vcf
		out_tableize=$sftp/$RunName/UnifiedGenotyper/$SampleID/$onTarget_BAM.final.UnifiedGenotyper.snp.VEP.vcf.table.gz
